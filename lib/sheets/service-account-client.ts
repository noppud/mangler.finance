...
export class ServiceAccountSheetsClient {
  private sheets: sheets_v4.Sheets;
  private auth: JWT;

  constructor() {
    // Load service account credentials
    const keyPath = path.join(process.cwd(), 'fintech-hackathon-478313-93c79ddbebac.json');
  ...
  private determineCellType(
    ...
    if (effectiveValue.stringValue !== undefined) return 'string';
    return 'empty';
  }
}
